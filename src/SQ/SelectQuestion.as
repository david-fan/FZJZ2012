...
public class SelectQuestion extends MSprite {
    ...
    private var _recordUI : RecordUI = RecordUI.getInstance("sq_record");
    public function SelectQuestion() {
        ...
        //
        _records = RecordUtil.readRecord("sq_record.xml");
    ...
    public function endGame():void {
        var record:Record = RecordUtil.ReturnTop5Record(_records, _ok);
        if (record.top5) {
            RecordUtil.saveRecord("sq_record.xml", record.records);
        }
        FinishAlert.show(record.top5, this);
    }

    private var _ok:int=0;

    private function onSubmit(e:MouseEvent):void {
        var ok:Boolean = bord.getCurrentQuestionOK();
        if (ok)
            _ok++;
        var alert : AlertUI = AlertUI.getInstance("sq-alert",playIndex);
        alert.rule.text = (ok?"回答正确":"回答错误");
        UtilManager.popUpUtil.addPopUp(alert,true);
    }
}
}